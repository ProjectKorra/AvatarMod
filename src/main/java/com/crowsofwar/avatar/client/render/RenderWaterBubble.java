...
public class RenderWaterBubble extends Render<EntityWaterBubble> {
	...
	public void doRender(EntityWaterBubble bubble, double x, double y, double z, float entityYaw,
		...
		mat = mat.translate((float) x - 0.5f, (float) y * bubble.getSize(), (float) z - 0.5f);
		...
		mat = mat.rotate(ticks / 25f * bubble.getDegreesPerSecond(), 1, 0, 0);
		mat = mat.rotate(ticks / 25f * bubble.getDegreesPerSecond(), 0, 1, 0);
		mat = mat.rotate(ticks / 25f * bubble.getDegreesPerSecond(), 0, 0, 1);
		...
		Vector4f
				...
				rbf = new Vector4f(bubble.getSize(), 0, 0, 1).mul(mat),
				ltf = new Vector4f(0, bubble.getSize(), 0, 1).mul(mat),
				rtf = new Vector4f(bubble.getSize(), bubble.getSize(), 0, 1).mul(mat),
				lbb = new Vector4f(0, 0, bubble.getSize(), 1).mul(mat),
				rbb = new Vector4f(bubble.getSize(), 0, bubble.getSize(), 1).mul(mat),
				ltb = new Vector4f(0, bubble.getSize(), bubble.getSize(), 1).mul(mat),
				rtb = new Vector4f(bubble.getSize(), bubble.getSize(), bubble.getSize(), 1).mul(mat);
