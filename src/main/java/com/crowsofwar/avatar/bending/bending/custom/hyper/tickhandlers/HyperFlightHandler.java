...
public class HyperFlightHandler extends TickHandler {

    public HyperFlightHandler(int id) {
        super(id);
    }
    ...
    public boolean tick(BendingContext ctx) {
        ...
        AbilityData data = ctx.getData().getAbilityData("hyper_flight");
        AbilityHyperFlight fly = (AbilityHyperFlight) Abilities.get("hyper_flight");
        ...
        if (world.isRemote && fly != null) {
            ...
            for (int i = 0; i < 8 + AvatarUtils.getRandomNumberInRange(2, 4); i++) {
                ...
                ParticleBuilder.create(ParticleBuilder.Type.FLASH).clr(r, g, b, 215 + AvatarUtils.getRandomNumberInRange(0, 40))
                        .fade(getClrRand(), getClrRand(), getClrRand(), 160 + AvatarUtils.getRandomNumberInRange(0, 40))
                ...
                if (entity.getOwner() != null) {
                    ...
                    if (entity.ticksExisted <= ((EntityShockwave) entity).getParticleWaves()) {
                        ...
                        for (double angle = 0; angle < 2 * Math.PI; angle += Math.PI / (((EntityShockwave) entity).getRange() *
                            ...
                            ParticleBuilder.create(ParticleBuilder.Type.FLASH).vel(speed.toMinecraft())
                                    .spawnEntity(owner).collide(world.rand.nextBoolean()).collideParticles(world.rand.nextBoolean()).clr(rgb[0], rgb[1], rgb[2], 180 + AvatarUtils.getRandomNumberInRange(0, 40)).
                                    fade(getClrRand(), getClrRand(), getClrRand(), 160 + AvatarUtils.getRandomNumberInRange(0, 40)).pos(x2, y2, z2).
                            ...
                            ParticleBuilder.create(ParticleBuilder.Type.FLASH).vel(speed.toMinecraft())
                                    .spawnEntity(owner).clr(rgb[0], rgb[1] * 8, rgb[2] * 4, 180 + AvatarUtils.getRandomNumberInRange(0, 40)).
                                    fade(getClrRand(), getClrRand(), getClrRand(), 160 + AvatarUtils.getRandomNumberInRange(0, 40)).pos(x2, y2, z2).
                                    scale(entity.getAvgSize() * 2).glow(AvatarUtils.getRandomNumberInRange(1, 100) > 25).time(time + AvatarUtils.getRandomNumberInRange(0, 2)).spawn(world);
                        }
                    }
