...
public class WaterSkateHandler extends TickHandler {
                    ...
                    if (world.isRemote) {
                        ...
                        for (int i = 0; i < particles * 4; i++) {
                            Vec3d pos = Vector.getEntityPos(player).plus(newVelocity.x(), 0, newVelocity.z())
                                    .plus(world.rand.nextGaussian() / 20, .1, world.rand.nextGaussian() / 20).toMinecraft();
                            ParticleBuilder.create(ParticleBuilder.Type.CUBE).element(new Waterbending())
                                    ...
                                    .vel(world.rand.nextGaussian() / 20, world.rand.nextDouble() * targetSpeed / 10,
                                            world.rand.nextGaussian() / 20).spawnEntity(player).spawn(world);
                        }
                    }
    ...
    public void onRemoved(BendingContext ctx) {
        super.onRemoved(ctx);
        AbilityData abilityData = ctx.getData().getAbilityData("water_skate");
        AbilityWaterSkate skate = (AbilityWaterSkate) Abilities.get("water_skate");

        if (abilityData != null && skate != null) {
            abilityData.setRegenBurnout(true);
            if (!(ctx.getBenderEntity() instanceof EntityPlayer && ((EntityPlayer) ctx.getBenderEntity()).isCreative()))
                abilityData.setAbilityCooldown(skate.getCooldown(abilityData));
        }
    }
