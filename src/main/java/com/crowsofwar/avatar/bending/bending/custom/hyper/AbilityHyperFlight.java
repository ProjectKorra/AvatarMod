...
import static com.crowsofwar.avatar.util.data.StatusControlController.HYPER_FLIGHT;
import static com.crowsofwar.avatar.util.data.TickHandlerController.HYPER_FLIGHT_HANDLER;

public class AbilityHyperFlight extends Ability {

    public AbilityHyperFlight() {
        super(Hyperbending.ID, "hyper_flight");
    }
    ...
    public void execute(AbilityContext ctx) {
        ...
        if (!data.hasTickHandler(HYPER_FLIGHT_HANDLER) && !ctx.getWorld().isRemote) {
            if (!data.hasStatusControl(HYPER_FLIGHT) && bender.consumeChi(getChiCost(abilityData) / 8)) {

                data.addStatusControl(HYPER_FLIGHT);
                if (data.hasTickHandler(HYPER_FLIGHT_HANDLER)) {
                    StatusControl sc = HYPER_FLIGHT;
            ...
            data.removeTickHandler(HYPER_FLIGHT_HANDLER, ctx);
            if (data.hasStatusControl(HYPER_FLIGHT))
                data.removeStatusControl(HYPER_FLIGHT);
        }
    }
