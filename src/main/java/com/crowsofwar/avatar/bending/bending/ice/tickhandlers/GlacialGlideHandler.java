...
import static com.crowsofwar.avatar.bending.bending.Ability.FADE_B;
import static com.crowsofwar.gorecore.util.Vector.toRectangular;
import static java.lang.Math.toRadians;

public class GlacialGlideHandler extends TickHandler {

    public GlacialGlideHandler(int id) {
        super(id);
    }
    ...
    public boolean tick(BendingContext ctx) {
        ...
        AbilityData data = ctx.getData().getAbilityData(new AbilityGlacialGlide());
        AbilityGlacialGlide fly = (AbilityGlacialGlide) Abilities.get(new AbilityGlacialGlide().getName());
        ...
        if (world.isRemote && fly != null) {
            ...
            for (int i = 0; i < 8 + AvatarUtils.getRandomNumberInRange(2, 4); i++) {
                ...
                ParticleBuilder.create(ParticleBuilder.Type.FLASH).clr(r, g, b, 215 + AvatarUtils.getRandomNumberInRange(0, 40))
                        ...
                        .scale(size).time(6 + AvatarUtils.getRandomNumberInRange(0, 6)).element(BendingStyles.get(Icebending.ID)).collide(true)
    ...
    public static class GlacialGlideShockwave extends OffensiveBehaviour {
                ...
                if (entity.getOwner() != null) {
                    ...
                    if (entity.ticksExisted <= ((EntityShockwave) entity).getParticleWaves()) {
                        ...
                        for (double angle = 0; angle < 2 * Math.PI; angle += Math.PI / (((EntityShockwave) entity).getRange() *
                            ...
                            int rRandom = AvatarUtils.getRandomNumberInRange((int) (fade[0] * 0.875), (int) (fade[0] * 1.125));
                            int gRandom = AvatarUtils.getRandomNumberInRange((int) (fade[1] * 0.875), (int) (fade[1] * 1.125));
                            int bRandom = AvatarUtils.getRandomNumberInRange((int) (fade[2] * 0.875), (int) (fade[2] * 1.125));
                            ...
                            ParticleBuilder.create(ParticleBuilder.Type.FLASH).element(BendingStyles.get(Icebending.ID)).vel(speed.toMinecraft())
