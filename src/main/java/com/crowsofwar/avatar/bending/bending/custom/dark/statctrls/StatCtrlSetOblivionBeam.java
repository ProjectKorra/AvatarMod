...
import static com.crowsofwar.avatar.util.data.TickHandlerController.OBLIVION_BEAM_CHARGER;
import static com.crowsofwar.avatar.util.data.TickHandlerController.OBLIVION_BEAM_HANDLER;

/**
 * @author CrowsOfWar
 */
public class StatCtrlSetOblivionBeam extends StatusControl {

    private final boolean setting;

    public StatCtrlSetOblivionBeam(boolean setting) {
    ...
    public boolean execute(BendingContext ctx) {
        ...
        if (data.hasBendingId(Darkbending.ID)) {
            if (setting) {
                data.addStatusControl(STOP_OBLIVION_BEAM);
                data.addTickHandler(OBLIVION_BEAM_CHARGER, ctx);
            } else {
                if (bender.getEntityAttribute(SharedMonsterAttributes.MOVEMENT_SPEED).getModifier(OBLIVION_BEAM_MOVEMENT_MOD_ID) != null)
                    bender.getEntityAttribute(SharedMonsterAttributes.MOVEMENT_SPEED).removeModifier(OBLIVION_BEAM_MOVEMENT_MOD_ID);
                data.removeTickHandler(OBLIVION_BEAM_HANDLER, ctx);
                data.removeTickHandler(OBLIVION_BEAM_CHARGER, ctx);
            }
        }
