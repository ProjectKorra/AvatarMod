    ...
    public boolean tick(BendingContext ctx) {
        ...
        if (world.isRemote) {
            for (int i = 0; i < 10 + Math.max(aD.getLevel(), 1) * 2; i++) {
                ...
                ParticleBuilder.create(ParticleBuilder.Type.FLASH).spawnEntity(entity).pos(location.plus(Vector.getEntityPos(entity)).toMinecraft()).time(4 + AvatarUtils.getRandomNumberInRange(1, 4)).
