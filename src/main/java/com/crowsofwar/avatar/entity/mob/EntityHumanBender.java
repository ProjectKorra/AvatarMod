...
import static com.crowsofwar.avatar.network.AvatarChatMessages.MSG_NEED_TRADE_ITEM;
...
public abstract class EntityHumanBender extends EntityBender implements IMerchant, INpc {
    ...
    private ItemStack getRandomPrice(int tier) {
        ...
        String element = BendingStyles.get(getElement()).getName();
        switch (element) {
            case "airbending":
    ...
    private ItemStack getRandomItemOfTier(int tier) {
        ...
        else toSell = new ItemStack(Scrolls.getTypeFromElement(BendingStyles.get(getElement()).getName()), 1, tier - 1);
    ...
    private void generateRecipes() {
        if (trades == null) {
            ...
            ItemStack elementScroll = new ItemStack(Scrolls.getTypeFromElement(BendingStyles.get(getElement())), 1, 2);
            ItemStack staff;
