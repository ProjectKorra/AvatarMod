...
public class AbilityConfig {
	...
	public static void loadWorldSpecificSpellProperties(World world) {
		...
		AvatarLog.info("Loading custom spell properties for world {}", world.getWorldInfo().getWorldName());
	...
	private static boolean loadConfigAbilityProperties() {
		...
		return loadAbilityPropertiesFromDir(spellJSONDir);
	}
	...
	private static boolean loadBuiltInAbilityProperties() {
		...
		boolean success = CraftingHelper.findFiles(mod, "assets/" + AvatarInfo.MOD_ID + "/abilities", null,
				...
				(root, file) -> {
					...
					if (!abilities.remove(ability)) Wizardry.logger.warn("What's going on?!");
					...
					try {
						...
						SpellProperties properties = new SpellProperties(json, ability);
