 ...
 * if running from server. <br />
 * <br />
 */
@Mod.EventBusSubscriber
public abstract class AvatarCommonProxy {
	...
	/**
	 * Called during the FMLPreInitialization event, subclasses should initialize themselves here
	 * (fields, etc).
	 */
	public void preInit(FMLPreInitializationEvent event) {
		AvatarPlayerData.initFetcher(getClientDataFetcher());
		ConfigStats.load();
		ConfigSkills.load();
		ConfigClient.load();
		ConfigChi.load();
		...
		NetworkRegistry.INSTANCE.registerGuiHandler(AvatarMod.instance, new AvatarGuiHandler());
		...
		ForgeChunkManager.setForcedChunkLoadingCallback(AvatarMod.instance, (tickets, world) -> {});
	}
	...
	public void onServerStarting(FMLServerStartingEvent event) {
		event.registerServerCommand(new AvatarCommand());
	}
	/**
	 * Called during the FMLPostInitialization event
	 */
