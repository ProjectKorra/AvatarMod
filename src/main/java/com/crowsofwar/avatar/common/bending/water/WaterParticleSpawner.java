...
public class WaterParticleSpawner extends TickHandler {
	...
	public boolean tick(BendingContext ctx) {
		...
		double radius = (maxDuration - duration) / 10f;
		...
		if (data.hasTickHandler(WATER_CHARGE) && !world.isRemote) {
			...
			AxisAlignedBB box = new AxisAlignedBB(entity.posX + (1 * radius), entity.posY + 2, entity.posZ + (1 * radius), entity.posX - (1 * radius),
												  entity.posY - 2, entity.posZ - (1 * radius));
