		...
		if (event.getSource().getTrueSource() == entity && (entity instanceof EntityBender || entity instanceof EntityPlayer)) {
			...
			if (ctx.getData() != null) {
				...
				if (ctx.getData().hasStatusControl(INFERNO_PUNCH)) {
					System.out.println(((EntityLivingBase) entity).getHeldItemMainhand() == ItemStack.EMPTY && !(source.getDamageType()
									.startsWith("avatar_")));
					if (((EntityLivingBase) entity).getHeldItemMainhand() == ItemStack.EMPTY && !(source.getDamageType().startsWith("avatar_"))) {
						...
						if (world instanceof WorldServer || !world.isRemote) {
							WorldServer World = (WorldServer) target.getEntityWorld();
							World.spawnParticle(EnumParticleTypes.FLAME, target.posX, target.posY + target.getEyeHeight(), target.posZ, 50, 0.05,
												0.05, 0.05, 0.05);

						}

						world.playSound(null, target.posX, target.posY, target.posZ, SoundEvents.ENTITY_GHAST_SHOOT, SoundCategory.HOSTILE, 4.0F,
	...
	public static void onDragonHurt(LivingHurtEvent event) {
		...
		if (entity instanceof EntityPlayer || entity instanceof EntityBender) {
			...
			if (data != null) {
				...
				float damageModifier = (float) (ctx.calcPowerRating(Firebending.ID) / 100);
				float damage = STATS_CONFIG.InfernoPunchDamage + (2 * damageModifier);
				if (data.hasStatusControl(INFERNO_PUNCH) && !(event.getSource().getDamageType().equals("avatar_groundSmash")) && !(event.getSource()
								.getDamageType().equals("avatar_Air"))) {
