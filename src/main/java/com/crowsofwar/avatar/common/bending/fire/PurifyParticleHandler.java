	...
	public boolean tick(BendingContext ctx) {
		...
		if (!world.isRemote) {
			for (int i = 0; i < 18; i++) {
				...
				r = random == 1 ? r : r * -1;
				Vector location = Vector.toRectangular(Math.toRadians(entity.rotationYaw + (i * 20) + (r * 2)), 0).times(0.5)
								.withY(entity.getEyeHeight() - 0.7);
				particles.spawnParticles(world, EnumParticleTypes.FLAME, 1, 1, location.plus(Vector.getEntityPos(entity)), new Vector(0.6, 1.8, 0.6));
			}
		}
