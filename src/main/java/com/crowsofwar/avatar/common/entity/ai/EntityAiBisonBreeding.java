...
public class EntityAiBisonBreeding extends EntityAIBase {
	...
	public boolean shouldContinueExecuting() {
		...
		List<EntitySkyBison> mates = bison.world
						.getEntitiesWithinAABB(EntitySkyBison.class, aabb, b -> b != bison && b.getCondition().isReadyToBreed());
	...
	private void spawnBaby(EntitySkyBison mate) {
		...
		if (child != null) {
			...
			child.onInitialSpawn(world.getDifficultyForLocation(bison.getPosition()), new BisonSpawnData(true));
			...
			for (int i = 0; i < 7; ++i) {
				...
				world.spawnParticle(EnumParticleTypes.HEART, bison.posX + dx, bison.posY + dy, bison.posZ + dz, mx, my, mz);

			}

			// Spawn XP orbs
			if (world.getGameRules().getBoolean("doMobLoot")) {
				world.spawnEntity(new EntityXPOrb(world, bison.posX, bison.posY, bison.posZ, random.nextInt(7) + 1));
			}
	...
	private int getNearbyBison() {
		...
		AxisAlignedBB aabb = new AxisAlignedBB(bison.posX - 32, 0, bison.posZ - 32, bison.posX + 32, 255, bison.posZ + 32);
