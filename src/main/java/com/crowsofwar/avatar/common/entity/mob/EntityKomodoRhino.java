 ...
 */
public class EntityKomodoRhino extends EntityAnimal implements IInventoryChangedListener {

    private static final DataParameter<Float> SYNC_RIDE_SPEED = EntityDataManager
            .createKey(EntityKomodoRhino.class, DataSerializers.FLOAT);

    private static final DataParameter<Integer> SYNC_EQUIPMENT = EntityDataManager.createKey
            (EntityKomodoRhino.class, DataSerializers.VARINT);
     ...
     * @param world
     */
    public EntityKomodoRhino(World world) {
        super(world);
        setSize(1.5f, 1.5f);
        setupChest();
    }
    ...
    private boolean processInteractOnce(EntityPlayer player) {
        ...
        for (EnumHand hand : EnumHand.values()) {
            ...
            if (heldStack.getItem() == AvatarItems.itemKomodoRhinoEquipment) {

                ItemKomodoRhinoEquipment.EquipmentTier proposed = ItemKomodoRhinoEquipment.EquipmentTier
    ...
    @Nullable
    public ItemKomodoRhinoEquipment.EquipmentTier getEquipment() {
        int index = dataManager.get(SYNC_EQUIPMENT);
        return ItemKomodoRhinoEquipment.EquipmentTier.getTier(index);
    }
    ...
    private void dropEquipment(boolean dropItem) {
        if (getEquipment() != null) {
            ...
            if (dropItem) {
                ...
                ItemStack stack = new ItemStack(AvatarItems.itemKomodoRhinoEquipment, 1, index);
