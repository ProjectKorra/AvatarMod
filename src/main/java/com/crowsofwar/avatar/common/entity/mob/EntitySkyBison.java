...
public class EntitySkyBison extends EntityBender implements IEntityOwnable, IInventoryChangedListener {
	...
	protected void initEntityAI() {
		...
		targetTasks.addTask(2, new EntityAiBisonDefendOwner(this));
		targetTasks.addTask(3, new EntityAiBisonHelpOwnerTarget(this));

		tasks.addTask(4, Objects.requireNonNull(Abilities.get("air_bubble")).getAi(this, getBender()));
		tasks.addTask(1, Objects.requireNonNull(Abilities.get("air_gust")).getAi(this, getBender()));
		tasks.addTask(3, Objects.requireNonNull(Abilities.get("airblade")).getAi(this, getBender()));

		tasks.addTask(2, new EntityAiBisonFollowAttacker(this));
		tasks.addTask(3, new EntityAiBisonSit(this));
		tasks.addTask(4, new EntityAiBisonBreeding(this));
		tasks.addTask(5, new EntityAiBisonTempt(this, 10));
		tasks.addTask(6, aiEatGrass = new EntityAiBisonEatGrass(this));
		tasks.addTask(6, new EntityAiBisonLand(this));
		tasks.addTask(7, new EntityAiBisonFollowOwner(this));
		tasks.addTask(8, new EntityAILookIdle(this));
		tasks.addTask(9, new EntityAiBisonWander(this));
	...
	public void updatePassenger(Entity passenger) {
		...
		if (index > -1) {
			...
			if (passenger == getControllingPassenger() && !isSitting()) {
			...
			if (passenger != getControllingPassenger()) {
				if (motionX != 0 || motionY != 0 || motionZ != 0) {
					passenger.rotationYaw = rotationYawHead;
	...
	public void onUpdate() {
		...
		if (isSitting() && hasOwner() && (world.getBlockState(getEntityPos(this).toBlockPos()).getBlock() != Blocks.AIR)) {
		...
		if (world.isRemote && isLoveParticles() && ticksExisted % 10 == 0) {
			...
			world.spawnParticle(EnumParticleTypes.HEART, posX + rand.nextFloat() * width * 2 - width, posY + 0.5D + rand.nextFloat() * height,
								posZ + rand.nextFloat() * width * 2 - width, d0, d1, d2, new int[0]);
		...
		if (!madeSitByPlayer && isSitting() && condition.getFoodPoints() > 0) {
			setSitting(false);
		}
	...
	public void travel(float strafe, float jump, float forward) {
		...
		if (isBeingRidden() && canBeSteered()) {
			...
			float current = normalizeAngle(rotationYaw);
			...
			rotationYawHead = driver.rotationYaw;
			...
			limbSwingAmount += (moveSinceLastTick - limbSwingAmount) * 0.4F;
	...
	private void travelFlying(float strafe, float jump, float forward) {
		if (isInWater()) {
			moveRelative(strafe, jump, forward, 0.02F);
			...
			if (onGround) {
				f = world.getBlockState(
								new BlockPos(MathHelper.floor(posX), MathHelper.floor(getEntityBoundingBox().minY) - 1, MathHelper.floor(posZ)))
								.getBlock().slipperiness * 0.91F;
			}

			float f1 = 0.16277136F / (f * f * f);
			moveRelative(strafe, jump, forward, onGround ? 0.1F * f1 : 0.06F);
			f = 0.91F;
	...
	protected void playTameEffect(boolean success) {
		...
		for (int i = 0; i < 7; i++) {
			...
			world.spawnParticle(particle, posX + rand.nextFloat() * width * 2.0F - width, posY + 0.5D + rand.nextFloat() * height,
