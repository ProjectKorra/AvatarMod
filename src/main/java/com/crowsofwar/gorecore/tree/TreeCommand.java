...
public abstract class TreeCommand implements ICommand {
	...
	public TreeCommand(MessageConfiguration cfg) {
		...
		branchRoot = new NodeBranch(newChatMessage(cfg, "gc.tree.branchHelp.root", "command"), getName(), addCommands());
	...
	@Override
	public List<String> getTabCompletions(MinecraftServer server, ICommandSender sender, String[] sentArgs, BlockPos pos) {
		...
		while (node != null) {
			...
			if (node instanceof NodeBranch && call.getArgumentsLeft() > 1) {
				...
				return suggestions.stream().filter(suggest -> suggest.toLowerCase().startsWith(lastArg.toLowerCase())).collect(Collectors.toList());
