...
public class ArgumentPlayerName implements IArgument<String> {
	...
	public List<String> getCompletionSuggestions(ICommandSender sender, String currentInput) {
		...
		world.playerEntities.forEach(player -> suggestions.add(player.getName()));

		if (!suggestions.get(0).toLowerCase().startsWith(currentInput.toLowerCase())) return new ArrayList<>();
		return suggestions;
	}
