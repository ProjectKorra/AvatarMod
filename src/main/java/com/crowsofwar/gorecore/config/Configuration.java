...
public class Configuration {
	 ...
	 *
	 * @param key String key to fetch value from
	 */
	 ...
	 *
	 * @param path Path to default configuration.
	 * @return this
	 */
	public Configuration withDefaults(String path) {
		try {
			...
			if (instr == null) throw new FileNotFoundException("Default configuration file not found: " + path);
			BufferedReader br = new BufferedReader(new InputStreamReader(instr, StandardCharsets.UTF_8));

			String ln;
			while ((ln = br.readLine()) != null) text += ln + "\n";
