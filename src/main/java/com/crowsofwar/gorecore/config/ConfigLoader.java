...
public class ConfigLoader {
	 ...
	 *
	 * @param path Path starting at ".minecraft/config/"
	 * @throws ConfigurationException when an error occurs while trying to read the file
	 */
	private static Map<String, Object> loadMap(String path) {
		...
		try {
			...
			GoreCore.LOGGER.error("Error while loading config at 'config/" + path + "':", ex);
		}
	 ...
	 * @param path              Path to the configuration file, from ".minecraft/config/"
	...
	public static void load(Object obj, String path, boolean neverIgnoreConfig) {
		...
		if (!neverIgnoreConfig) {
			...
			if (ignoreObject instanceof Boolean) {
	...
	private <T> void loadField(Field field) {
		...
		try {
			...
			if (field.getAnnotation(Load.class) != null) {
				...
				CustomLoaderSettings loaderInfo = loaderAnnot == null ? new CustomLoaderSettings() : new CustomLoaderSettings(loaderAnnot);
				...
				if (tryDefaultValue) {
					...
					setTo = convert(fromData, fieldType, field.getName());
				}
				...
				try {
					...
					if (loaderInfo.hasCustomLoader()) loaderInfo.customLoaderClass.newInstance().load(null, setTo);

				} catch (InstantiationException | IllegalAccessException e) {

					throw new ConfigurationException.ReflectionException(
									"Couldn't get a loader class of loader " + loaderInfo.customLoaderClass.getName(), e);
					...
					throw new ConfigurationException.Unexpected(
									"An unexpected error occurred while using a custom object loader from config. Offending loader is: "
													+ loaderInfo.customLoaderClass, e);
			...
			throw new ConfigurationException.Unexpected(
							"An unexpected error occurred while loading field \"" + field.getName() + "\" in class \"" + cls.getName() + "\"", e);
	 ...
	 * @param name   If there are no converters, a new object must be created
	 *               and @Load fields are populated. Will then use map from
	 *               {@link #data} with that name.
	 * @param <T>    The type which we must convert to
	 */
	private <T> T convert(Object object, Class<T> to, String name) {
		...
		if (from == to) {
			...
			try {
				...
				ConfigLoader loader = new ConfigLoader(path, loadedObject, (Map) data.get(name), ignoreConfigFile, neverIgnoreConfig);
				loader.load();
	...
	private String dump() {
		...
		try {
			...
			throw new ConfigurationException.Unexpected(
							"Unexpected error while trying to convert values to YAML: classTags " + classTags + ", values " + usedValues, e);
		}
	...
	private static class CustomLoaderSettings {
		...
		private CustomLoaderSettings(HasCustomLoader annot) {
			customLoaderClass = annot.loaderClass();
			loadFields = annot.loadMarkedFields();
		}
