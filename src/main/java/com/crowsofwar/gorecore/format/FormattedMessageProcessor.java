...
public class FormattedMessageProcessor {
	 ...
	 *
	 * @param msg          Information about how to format the message
	 * @param text         Text to format - should be already translated
	 * @param formatValues Values of the formatting arguments to use
	 */
				...
				if (tag.equals("bold")) {
					...
					if (tag.substring(1).equals(format.topFormat().name)) {
						...
						throw new ProcessingException(
										"Error processing message; closing tag does not match last opened tag: [" + tag.substring(1) + "]; text is: "
														+ text);
					}
		...
		while (matcher.find()) {
			...
			if (item.startsWith("[") && item.endsWith("]")) {
				...
				if (tag.startsWith("translate=")) {
					...
					String key = tag.substring("keybinding=".length());
					item = GoreCore.proxy.getKeybindingDisplayName(key);
				} else if (tag.equals("bold") || tag.equals("italic") || getTfColor(tag) != null || cfg.hasColor(tag) || tag.startsWith("/")) {
					formattingTag = true;
				} else {
					throw new ProcessingException("String has invalid tag: [" + tag + "]; text is " + text);
				}
